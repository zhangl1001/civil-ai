...
interface RunRow extends SqlRow {
  id: string; run_type: AgentRunType; status: AgentRunStatus; exam_cycle_id: string | null; learning_thread_id: string | null;
...
export class SqliteAgentRunRepository implements AgentRunRepository {
  ...
  async claimRunnable(options: AgentRunClaimOptions): Promise<readonly AgentRunAggregate[]> {
    ...
    return this.database.transaction(async (tx) => {
      const rows = await tx.query<RunRow>(`SELECT * FROM tutor_agent_runs WHERE status = 'queued' AND (next_run_at IS NULL OR next_run_at <= ?) ORDER BY updated_at, id LIMIT ?`, [options.now, options.limit]);
