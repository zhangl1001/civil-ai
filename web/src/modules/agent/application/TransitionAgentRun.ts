...
export class TransitionAgentRun {
  ...
  async execute(command:TransitionAgentRunCommand):Promise<AgentRunAggregate>{
    ...
    await this.unitOfWork.run(async context=>{await this.repository.replace(run,aggregate.run.version,event,context,command.leaseToken?{leaseToken:command.leaseToken,now}:undefined);await this.outbox.append({id:this.ids.next('OutboxEventId'),aggregateType:'tutor_agent_run',aggregateId:run.id,eventType:`tutor_agent_run.${event.eventType}`,payload:{agentRunId:run.id,status:run.status},occurredAt:now,attemptCount:0,idempotencyKey:`${command.idempotencyKey}:outbox`},context);}); return {run,events:[...aggregate.events,event]};
  }
}
