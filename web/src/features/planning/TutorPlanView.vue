    ...
    <PullToRefresh class="tutor-plan-content" :on-refresh="load">
      ...
      <template v-else>
        <section class="plan-summary">
          <span>{{ localDate }}{{ plan ? ` · ${preparationHorizonLabel(plan.plan.decisionFactors)}` : '' }}</span>
          <strong>{{ plan ? planProgressLabel(plan) : `${availableMinutes} 分钟可用时间` }}</strong>
          <p>{{ plan?.plan.decisionSummary || '先根据真实练习和复习状态生成一份可执行计划。' }}</p>
        </section>

        <template v-if="planGroups.length">
          <section v-for="group in planGroups" :key="group.category" class="plan-group" :aria-label="group.label">
              ...
              <article v-for="entry in group.items" :key="entry.item.id" :class="['plan-item', entry.item.status]">
                <button class="plan-item-main" type="button" :disabled="openingItemId === entry.item.id || entry.item.status === 'completed' || entry.item.status === 'skipped'" @click="openItem(entry.item)">
                  ...
                  <span class="item-copy">
                    ...
                    <em>{{ entry.moduleName }} · {{ entry.item.targetMinutes }} 分钟{{ targetCountLabel(entry.item) }}{{ statusLabel(entry.item.status) }}</em>
                    <small v-if="entry.item.failureMessage">{{ entry.item.failureMessage }}</small>
                  </span>
                  <ChevronRightIcon />
                </button>
                <button v-if="entry.item.status === 'pending' || entry.item.status === 'in_progress'" class="plan-item-skip" type="button" aria-label="取消计划项" @click="cancelTarget = entry.item">
                  <BanIcon />
                </button>
              </article>
            </div>
...
const plan = ref<DailyPlanAggregate>(); const capabilityMeta = ref(new Map<string, DailyPlanCapabilityMeta>()); const cycleLabel = ref('备考计划');
...
const planGroups = computed(() => plan.value ? buildDailyPlanGroups(plan.value, capabilityMeta.value) : []);
...
async function load() {
  ...
  try {
    ...
    capabilityMeta.value = state.capabilityMeta;
...
async function cancelItem() {
  ...
  const updatedPlan = await (await feature()).cancel(item.id);
  if (updatedPlan) plan.value = updatedPlan;
}
