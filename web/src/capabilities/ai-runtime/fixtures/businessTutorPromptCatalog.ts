...
export const businessTutorPromptCatalog: readonly PromptBundle[] = [
  bundle({
    ...
    sections: [
      ...
      section(PromptSectionCode.InputContract, '输入规格', 30, '输入包含主题、题型、小问数量和 generationVariation。不得编造官方真题来源；缺少可靠来源时视为 AI 生成内容。generationVariation 只用于避免与近期内容重复，不得改变题型和结构合同。'),
  ...
  bundle({
    ...
    sections: [
      ...
      section(PromptSectionCode.InputContract, '输入规格', 30, '输入包含日期、积累类型、具体要求、可选 learningFocus 和 generationVariation。type=news 时还必须包含 webEvidence、来源编号和 URL，只能以这些检索证据支持时效性事实；证据冲突、日期不明或来源身份不清时保守表述，禁止靠模型记忆补造政策名称、日期和精确数据。type=tips 时优先从 learningFocus 选择当天最值得学习的细分能力，侧重概念边界、识别方法、例子和易错点。generationVariation 用于变化教学视角并避开 recentOutlinesToAvoid，但不得偏离学习重点或事实证据。'),
      section(PromptSectionCode.OutputContract, '输出合同', 40, '只输出 GFM Markdown。每个主题使用一个二级标题；主题内部根据内容选择有价值的三级标题、段落、列表、引用或表格。type=news 的事实句使用 [来源N] 标注，末尾增加“## 参考来源”，逐条保留来源标题与完整 URL，不得生成输入中不存在的来源。数学公式仅在比纯文字更清晰时使用 KaTeX 兼容 LaTeX：行内公式使用 $...$，独立公式使用 $$...$$，百分号必须写成 \\%，不得用代码块或反引号包裹公式。Markdown 标记必须使用半角字符和真实换行，禁止代码围栏、JSON 包装、转义的 \\n、转义的标题符号或全角表格竖线。主题数量、栏目、篇幅和例子数量由信息价值与当天学习负担决定，适合手机分段阅读，不为凑固定结构重复内容。'),
  ...
  bundle({
    ...
    sections: [
      ...
      section(PromptSectionCode.InputContract, '输入规格', 30, '输入包含月份、按日期排列的每日热点和 generationVariation。重复事件应合并，证据不足的判断必须保守；变化复盘组织视角时不得改变输入事实。'),
