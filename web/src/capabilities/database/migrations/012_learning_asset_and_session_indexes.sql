ALTER TABLE learning_assets RENAME TO learning_assets_legacy;
...
CREATE TABLE learning_assets (
  ...
  kind TEXT NOT NULL CHECK(kind IN (
    ...
    'digest_monthly','study_lecture','mock_manifest','profile_insight'
  )),
...
INSERT INTO learning_assets(
  id,exam_cycle_id,kind,business_key,title,status,payload_json,source_agent_run_id,
  version,created_at,updated_at
)
SELECT
  id,exam_cycle_id,kind,business_key,title,status,payload_json,source_agent_run_id,
  version,created_at,updated_at
FROM learning_assets_legacy;

DROP TABLE learning_assets_legacy;
...
CREATE INDEX learning_sessions_question_set_idx
  ON learning_sessions(question_set_id,completed_at DESC,id DESC);
