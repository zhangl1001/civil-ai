...
运行预算按激活 Skill 在 compact / standard / research / long-running 四档中动态选择，并依据真实新证据受控扩容；全局硬上限为 32 个模型 turn、64 次工具调用和 15 分钟。相同工具与参数成功后禁止重复消费；失败或空结果会以结构化 observation 回送模型，可恢复失败允许一次原参数重试，连续无进展后必须由模型调整参数、范围、工具或步骤。每轮保存检查点，取消和切后台后不依赖内存数组恢复。
...
| 风险 | 防线 |
|---|---|
| 工具死循环 | 成功签名去重、可恢复失败受控重试、无进展换策略、动态预算硬上限、检查点 |
