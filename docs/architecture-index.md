...
| [核心底座交接](./core-foundation-handoff.md) | WP0-WP3 已完成能力、运行入口、不变量与下一批任务 | 生效 |
| [剩余交付任务清单](./remaining-delivery-checklist.md) | 当前未完成功能、验收顺序与执行入口 | 执行中 |

## 3. 历史文档

- `mobile-vue-capacitor-migration-plan.md`：仅用于了解 Vue/Capacitor 已迁移功能和现有代码位置。
- `../公考辅导-iOS改造计划.md`：仅用于核对早期功能清单和 iOS 问题背景。

历史文档中的旧数据兼容、文件驱动、后端数据主导、渐进保留旧模型等决策均不再执行。
...
- 数据切换：新数据库 `zhangl-agent-tutor-v1`，不导入旧业务数据。
- 业务中心：考试周期、能力图谱、学习证据、学习主线和独立掌握验证。
- UI 中心：首页是计划中心，AI 是持续私教，题目与内容使用版本化模板。

## 5. 实施规则

当前执行入口：首个 Vue Practice Feature Adapter 和削弱论证页面纵向切片。WP0-WP3 已完成；WP4 后端事实链路、WP5 掌握轨迹/快照/复习基础和 WP6 Agent Run/三并发/租约恢复/退避 worker 已落地。下一步不得继续扩旧 Service，而要让页面通过新 runtime 完成“主线 → 生题 → 作答 → 证据 → 掌握投影 → 下一动作”；接手前先读 [核心底座交接](./core-foundation-handoff.md)。

1. 先完成一个能力缺口的端到端纵向切片，再扩科目和页面。
