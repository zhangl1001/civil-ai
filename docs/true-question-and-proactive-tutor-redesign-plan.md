...
- `research.true_questions` 在聊天侧只装配 `research_true_questions` 任务派发工具；联网真题研究作为独立 `AgentRun` 进入内容工作池，不消耗对话轮次、工具数和超时预算。
- 独立 `TrueQuestionResearchAgent` 装配搜索、网页读取和扫描工具，使用长任务动态预算、检查点、取消与恢复；一次空结果后必须调整查询策略，只有获得真实 `draftId` 才算完成。
- 搜索结果必须进入现有导入草稿，用户在真题页面明确确认后才能发布；搜索工具和研究 Agent 都不能直接写正式题库。
...
自动化已验收：普通聊天不暴露搜索工具；真题聊天 Skill 只派发独立任务；研究 Agent 独占搜索、读页和扫描工具；三种搜索适配器、结果限额、内网拦截、只读工具并行/有序回填、观察器串行写入、草稿确认发布和真题统一管线均有回归测试。国内网络可达性、真实可选搜索凭证和来源展示仍需在 iPhone 候选包人工验收。
